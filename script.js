...
let modalQt = 1
let modalQtdEsfiha = 1
let modalQtdBebida = 1
...
function closeModal() {
    ...
    c('.esfihaWindowArea').style.opacity = 0
    setTimeout(() => {
        c('.esfihaWindowArea').style.display = 'none'
    })

}


cs('.cancelButton, .pizzaInfo--cancelMobileButton').forEach((item) => {
...
//no identifier especificar o tipo, se é esfiha ou bebida ou pizza

c('.esfihaInfo--addButton').addEventListener('click', () => {
    let size = c('.esfihaInfo--size.selected').getAttribute('data-key')
...
//no identifier especificar o tipo, se é esfiha ou bebida ou pizza
c('.bebidaInfo--addButton').addEventListener('click', () => {
    let size = c('.bebidaInfo--size.selected').getAttribute('data-key')    
...
//esfiha add qtd no modal e diminui qtd no modal

c('.esfihaInfo--qtmais').addEventListener('click', () => {
    modalQtdEsfiha++
    c('.esfihaInfo--qt').innerHTML = modalQtdEsfiha
})

c('.esfihaInfo--qtmenos').addEventListener('click', () => {
    if (modalQtdEsfiha > 1) {
        modalQtdEsfiha--
        c('.esfihaInfo--qt').innerHTML = modalQtdEsfiha
    }
...
function updateCart() {
    ...
    if (cart.length > 0) {
        ...
        for (let i in cart) {
            //fazer uma logica para verificar qual item esta sendo adicionado no carrinho no momento
            //depois atualizar com o preco dos itens que ja foram adicionados posteriormente
            
            let pizzaItem = pizzaJson.find((item) => item.id == cart[i].id)
            let esfihaItem = esfihaJson.find((item) => item.id == cart[i].identifier)
            let bebidaItem = bebidaJson.find((item) => item.id == cart[i].id)
            //verificar qual item esta sendo adicionado
